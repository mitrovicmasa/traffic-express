...
enum class Phase { WAGON_SELECTION, PHASE_1, PHASE_2 };
...
class Game : Serializable {
 public:
  ...
  std::vector<RoundCard *> selectRoundCards(
  ...
  std::vector<NeutralBullet *> generateNeutralBullets(
      unsigned numberOfNeutralBullets) const;
  Train *selectWagons(std::vector<Wagon *> allPossibleWagons,
  ...
  std::pair<Wagon *, Treasure *> actionRobbery(int treasureIndex,
                                               int wagonIndex);
  std::pair<Wagon *, Treasure *> actionPunch(int treasureIndex, int wagonIndex,
                                             int playerIndex);
