...
// Constructors
RoundCard::RoundCard(RoundCardType type, EventType event,
                     const MiniRoundCollection &miniRounds)
    : QGraphicsObject(),
      m_type(type),
      m_event(event),
...
RoundCard::RoundCard() { setFlags(GraphicsItemFlag::ItemIsSelectable); }

RoundCard::RoundCard(RoundCardType type, EventType event,
                     const std::vector<MiniRoundType> &miniRound)
    : QGraphicsObject(), m_type(type), m_event(event) {
...
// Get methods
RoundCardType RoundCard::typeOfRoundCard() const { return m_type; }

MiniRoundCollection &RoundCard::getMiniRounds() { return m_miniRounds; }

EventType RoundCard::event() const { return m_event; }

// Set methods
void RoundCard::setType(RoundCardType &newType) { m_type = newType; }

void RoundCard::setEvent(EventType &newEvent) { m_event = newEvent; }
...
void RoundCard::push_back(MiniRound *mr) {
  ...
  mr->setPos((size() - 1) * (mr->width() + 10) * 0.8,
             height() - mr->height() - 10);
}

int RoundCard::size() const { return m_miniRounds.size(); }

MiniRound *RoundCard::operator[](int i) { return m_miniRounds[i]; }

MiniRound *RoundCard::back() { return m_miniRounds.back(); }

MiniRound *RoundCard::front() { return m_miniRounds.front(); }
...
// GUI
int RoundCard::height() const { return 100; }

int RoundCard::width() const { return 300; }

QRectF RoundCard::boundingRect() const {
  return QRectF(0, 0, width(), height());
}

void RoundCard::paint(QPainter *painter, const QStyleOptionGraphicsItem *option,
                      QWidget *widget) {
