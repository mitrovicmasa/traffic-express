...
PlayerStats::PlayerStats()
  ...
  for (Treasure *t : m_player->treasure()) {
    ...
    t->setPos(i * 30 + 80, 20);
    i++;
    connect(t, &Treasure::clickedTreasure, this,
            &PlayerStats::onTreasureClicked);
  }
}

PlayerStats::PlayerStats(Player *p, bool deepCopy) : QGraphicsObject() {
  if (!deepCopy) {  // shallow copy
    ...
    m_player = new Player(p->isItMyMove(), p->id(), p->hand(), p->deck(),
                          p->bullet_deck(), p->positionInTrain(), p->roof(),
                          p->treasure());
    int i = 0;
    for (Treasure *t : m_player->treasure()) {
      t->setParentItem(this);
...
// Get methods
Player *PlayerStats::getPlayer() { return m_player; }
...
Treasure *PlayerStats::takeTreasureFromPlayer(Treasure *t) {
  for (auto it = this->getPlayer()->treasure().begin();
       it != this->getPlayer()->treasure().end(); it++) {
    if (t == *it) {
      disconnect(t, &Treasure::clickedTreasure, this,
                 &PlayerStats::onTreasureClicked);
...
void PlayerStats::repositionTreasure() {
  for (int i = 0; i < m_player->treasure().size(); i++) {
    m_player->treasure()[i]->setPos(
        15 + (i) * (1.2 * (m_player->treasure()[i]->width())), 20);
  }
}

// GUI
QRectF PlayerStats::boundingRect() const { return QRectF(0, 0, 380, 70); }

void PlayerStats::paint(QPainter *painter,
                        const QStyleOptionGraphicsItem *option,
                        QWidget *widget) {
  Q_UNUSED(option)
  Q_UNUSED(widget)

  painter->drawPixmap(boundingRect(), QPixmap("://button.png"),
                      QRectF(0, 0, 0, 0));
  ...
  painter->drawText(boundingRect(), "    " + bandit + " - Number of bullets: " +
                                        QString::number(num));
}
