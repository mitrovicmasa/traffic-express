...
// Get methods
CardColection &Hand::getCards() { return m_cards; }
...
Card *Hand::back() { return m_cards.back(); }

Card *Hand::front() { return m_cards.front(); }

bool Hand::empty() { return m_cards.empty(); }

int Hand::size() { return m_cards.size(); }

Card *Hand::operator[](int ind) { return m_cards[ind]; }
...
// GUI
QRectF Hand::boundingRect() const { return QRectF(0, 0, 700, 120); }

void Hand::paint(QPainter *painter, const QStyleOptionGraphicsItem *option,
                 QWidget *widget) {
  painter->fillRect(boundingRect(), QColor(210, 222, 230));
  painter->drawPixmap(boundingRect(), QPixmap("://hands.png"),
                      QRectF(0, 0, 0, 0));
}
...
void Hand::onClickedCard(Card *c) { emit clickedCardInHand(c, this); }
