...
// Constructors
Wagon::Wagon(TreasureChest &upt, TreasureChest &downt, PlayerGroup &upp,
...
Wagon::Wagon() { setFlags(GraphicsItemFlag::ItemIsSelectable); }
...
// Get methods
TreasureChest &Wagon::getContentDown() { return m_contentDown; }

TreasureChest &Wagon::getContentUp() { return m_contentUp; }

PlayerGroup &Wagon::getPlayersUp() { return m_playersUp; }

PlayerGroup &Wagon::getPlayersDown() { return m_playersDown; }

bool Wagon::isLocomotive() const { return m_isLocomotive; }
...
void Wagon::setContentDown(TreasureChest newContentDown) {
  ...
  for (Treasure *t : newContentDown) {
    ...
    t->setPos(10 + (m_contentDown.size() - 1) * (2 * t->width()),
              -30 + height() - t->height());
  }
}
...
void Wagon::setContentUp(TreasureChest newContentUp) {
  ...
  for (Treasure *t : newContentUp) {
    ...
    t->setParentItem(this);
    t->setPos(10 + (m_contentUp.size() - 1) * (2 * t->width()),
              -30 + height() - t->height());
  }
  repositionTreasure();
}
...
void Wagon::addContentUp(Treasure *t) {
  ...
  m_contentUp.push_back(t);
  t->setParentItem(this);
  t->setPos(15 + (m_contentUp.size() - 1) * (1.2 * t->width()),
            70 - t->height());
  repositionTreasure();
}
...
void Wagon::addContentDown(Treasure *t) {
  ...
  m_contentDown.push_back(t);
  t->setParentItem(this);
  t->setPos(15 + (m_contentDown.size() - 1) * (1.2 * t->height()),
            -40 + height() - t->width());
  repositionTreasure();
}
...
void Wagon::addPlayerUp(Player *p) {
  ...
  p->setPos(5 + (p->width() + 5) * (m_playersUp.size() - 1),
            70 - p->height() - 25);
}
...
Treasure *Wagon::takeContentUp(TreasureType t) {
  ...
  for (int i = 0; i < m_contentDown.size(); i++) {
    if (m_contentDown[i]->getType() == t) tmp.push_back(m_contentDown[i]);
  }
  if (tmp.size() > 0) {
    int index = QRandomGenerator::global()->bounded((int)tmp.size());
    r = tmp[index];
    m_contentDown.erase(
        std::find(m_contentDown.begin(), m_contentDown.end(), r));
...
Player *Wagon::takePlayerUp(BanditType bandit) {
  ...
  for (auto x = m_playersUp.begin(); x != m_playersUp.end(); x++) {
    if ((*x)->id() == bandit) {
      ...
      m_playersUp.erase(x);
      disconnect(playerToRemove, &Player::clickedPlayer, this,
                 &Wagon::onClickedPlayer);
      repositionPlayers();
      break;
    }
  }
...
Treasure *Wagon::takeContentUp(Treasure *t) {
  for (auto it = m_contentUp.begin(); it != m_contentUp.end(); it++) {
    if (*it == t) {
      ...
      disconnect(*it, &Treasure::clickedTreasure, this,
                 &Wagon::onCickedTreasure);
...
int Wagon::numberOfTreasureInWagonDown(TreasureType type) const {
  return std::count_if(
      m_contentDown.begin(), m_contentDown.end(),
      [type](auto treasure) { return treasure->getType() == type; });
}
...
std::string Wagon::toString() {
  ...
  for (auto &begin : m_contentUp) r += begin->toString() + ", ";
  r += "\nMyContentDown:\n";
  for (auto &begin : m_contentDown) r += begin->toString() + ", ";
  return r;
}

// GUI

int Wagon::height() const { return 180; }

int Wagon::width() const { return 180; }

QRectF Wagon::boundingRect() const { return QRectF(0, 0, width(), height()); }

void Wagon::paint(QPainter *painter, const QStyleOptionGraphicsItem *option,
                  QWidget *widget) {
  if (this->isLocomotive())
    painter->drawPixmap(boundingRect(), QPixmap("://locomotive2.png"),
                        QRectF(0, 0, 0, 0));
  else
    painter->drawPixmap(boundingRect(), QPixmap("://wagon3.png"),
                        QRectF(0, 0, 0, 0));
}
...
void Wagon::repositionPlayers() {
  for (int i = 0; i < m_playersUp.size(); i++) {
    m_playersUp[i]->setPos(5 + (m_playersUp[i]->width() + 5) * (i),
                           70 - m_playersUp[i]->height() - 25);
  }
...
void Wagon::onClickedPlayer(Player *p) { emit clickedPlayerInWagon(p, this); }
