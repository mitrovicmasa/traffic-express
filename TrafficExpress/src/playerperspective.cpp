...
PlayerPerspective::PlayerPerspective(Game *game, int playerIndex,
                                     QObject *parent)
    : QGraphicsScene(parent),
      m_game(game),
      m_player((game->players())[playerIndex]),
      m_table(new Table()) {
  Train *train = m_game->wagons();
  connect(train, &Train::clickedTreasureInWagonInTrain, this,
          &PlayerPerspective::onClickedTreasureInWagonInTrain);
  connect(train, &Train::clickedPlayerInWagonInTrain, this,
          &PlayerPerspective::onClickedPlayerInWagonInTrain);
  connect(train, &Train::clickedWagonInTrain, this,
          &PlayerPerspective::onClickedWagonInTrain);

  for (Player *p : m_game->players()) {
    m_table->push_back(new PlayerStats(p));
  }
  connect(m_table, &Table::clickedTreasureInPlayerStatsInTable, this,
          &PlayerPerspective::onClickedTreasureInPlayerStatsInTable);
  connect(m_player, &Player::clickedCardInHandInPlayer, this,
          &PlayerPerspective::onClickedCardInHandInPlayer);
  connect(m_player, &Player::clickedCardInDeckInPlayer, this,
          &PlayerPerspective::onClickedCardInDeckInPlayer);
}
...
void PlayerPerspective::removeGameFromScene() {
  for (auto x : this->children()) x->setParent(nullptr);
}

void PlayerPerspective::drawCards(int n) { m_player->drawCards(n); }
...
int PlayerPerspective::getPlayerSize() { return m_game->players().size(); }

void PlayerPerspective::onClickedTreasureInWagonInTrain(Treasure *t, Wagon *w,
                                                        Train *train) {
  if (m_game->currentAction() == ActionType::ROBBERY &&
      m_player->isItMyMove() && m_game->phase() == Phase::PHASE_2 &&
...
void PlayerPerspective::onClickedCardInHandInPlayer(Card *c, Hand *h,
                                                    Player *p) {
...
void PlayerPerspective::onClickedCardInDeckInPlayer(Card *c, Deck *d,
                                                    Player *p) {
...
void PlayerPerspective::onClickedTreasureInPlayerStatsInTable(Treasure *t,
                                                              PlayerStats *ps,
                                                              Table *p) {
  if (m_game->currentAction() == ActionType::PUNCH && m_player->isItMyMove() &&
      m_game->phase() == Phase::PHASE_2 && m_game->actionPending() &&
      m_game->playerClicked() ==
          m_game->findPlayerById(ps->getPlayer()->id())) {
    ...
    emit actionPunchSignal(m_game->findPlayersTreasureIndex(t, playerIndex),
                           playerIndex, wagonIndex);
    return;
  }
}

void PlayerPerspective::onClickedPlayerInWagonInTrain(Player *p, Wagon *w,
                                                      Train *t) {
  if (m_game->currentAction() == ActionType::FIRE && m_player->isItMyMove() &&
      m_game->phase() == Phase::PHASE_2) {
    emit actionFireSignal(m_game->findPlayerById(p->id()));
  }

  if (m_game->currentAction() == ActionType::PUNCH && m_player->isItMyMove() &&
      m_game->phase() == Phase::PHASE_2 &&
      p->positionInTrain() == m_player->positionInTrain() &&
      p->id() != m_player->id() && p->roof() == m_player->roof()) {
...
void PlayerPerspective::onClickedWagonInTrain(Wagon *w, Train *train) {
  if (m_player->isItMyMove() && m_game->phase() == Phase::WAGON_SELECTION &&
    ...
    emit playerChoseWagon(m_game->findPlayerById(m_player->id()),
                          train->getWagonIndex(w));
  ...
  if (m_game->currentAction() == ActionType::FLOOR_CHANGE &&
      m_player->isItMyMove() && m_game->phase() == Phase::PHASE_2) {
    emit actionChangeFloorSignal(train->getWagonIndex(w));
  }

  if (m_game->currentAction() == ActionType::MOVE && m_player->isItMyMove() &&
      m_game->phase() == Phase::PHASE_2) {
    emit actionChangeWagonSignal(train->getWagonIndex(w));
  }

  if (m_game->currentAction() == ActionType::MARSHAL &&
      m_player->isItMyMove() && m_game->phase() == Phase::PHASE_2) {
...
void PlayerPerspective::onPlayerChoseWagon(int playerIndex, int wagonIndex) {
  ...
  if (m_game->findPlayerById(playerToSet->id()) ==
      m_game->players().size() - 1) {
...
void PlayerPerspective::onPlayerPlayedCard(int playerIndex, int cardIndex) {
  ...
  if (m_game->phase() != Phase::PHASE_2) m_game->setNextPlayerToMove();
}
...
void PlayerPerspective::onActionFireSignal(int playerIndex) {
  if (!m_game->actionFire(playerIndex)) {
    ...
    QString text = QString::fromStdString(
        ::toString(m_game->players()[playerIndex]->id()));
    text.append(" 's been shot by ");
    text.append(QString::fromStdString(
        ::toString(m_game->players()[m_game->getIndexOfPlayerToMove()]->id())));
    m_game->dialogueBox()->setText(text);
...
void PlayerPerspective::onActionRobberySignal(int treasureIndex,
                                              int wagonIndex) {
  PlayerStats *ps = m_table->getPlayerStats()[m_game->getIndexOfPlayerToMove()];
  auto [wagonToTakeTreasure, treasureToTake] =
      m_game->actionRobbery(treasureIndex, wagonIndex);
  ...
  if (m_game->players()[m_game->getIndexOfPlayerToMove()]->roof()) {
    ...
    ps->addTreasureToPlayer(
        wagonToTakeTreasure->takeContentDown(treasureToTake));
  }
...
void PlayerPerspective::onActionPunchSignal(int treasureIndex, int playerIndex,
                                            int wagonIndex) {
  PlayerStats *ps = m_table->getPlayerStats()[playerIndex];
  auto [wagonToUpdate, treasure] =
      m_game->actionPunch(treasureIndex, wagonIndex, playerIndex);
  ...
  // Putting message in dialogue box
  QString text =
      QString::fromStdString(::toString(m_game->players()[playerIndex]->id()));
