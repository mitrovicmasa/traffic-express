...
PlayerServer::PlayerServer(QObject *parent)
    : QObject{parent}
{
...
void PlayerServer::startServer()
{
    ...
    connect(m_server,&QTcpServer::newConnection,this,&PlayerServer::onNewConection);
    ...
    if(m_firstConnect[playerIndex]){
        ...
        for(int i=0;i<m_clients.size();i++){
            if(i!=playerIndex){
                ...
                this->sendMessage(m_clients[playerIndex],"ac:"+m_names[i]+":"+ready);
            }
        }
        ...
        if(message.startsWith("start:") ){
            ...
            for(int i=0;i<m_clients.size();i++){
                if(i!=playerIndex)
                    this->sendMessage(m_clients[i],"start:");
        ...
        if(message.startsWith("ready:") && !m_gameOngoing){
            ...
            for(int i=0;i<m_clients.size();i++){
                if(i!=playerIndex)
                    this->sendMessage(m_clients[i],"ready:"+QString::number(playerIndex));
            }
