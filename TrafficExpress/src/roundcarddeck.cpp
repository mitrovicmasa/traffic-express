...
// Constructors
RoundCardDeck::RoundCardDeck() = default;

RoundCardDeck::RoundCardDeck(std::vector<RoundCard *> rcs) {
  for (auto it = rcs.crbegin(); it != rcs.crend(); it++) {
    RoundCard *tmp = *it;
    this->push_front(new RoundCard(tmp->typeOfRoundCard(), tmp->event(),
                                   tmp->getMiniRounds()));
  }
}
...
// Get methods
RoundCardCollection &RoundCardDeck::getRoundCards() { return m_rounds; }
...
RoundCard *RoundCardDeck::operator[](int i) { return m_rounds[i]; }

RoundCard *RoundCardDeck::back() { return m_rounds.back(); }

RoundCard *RoundCardDeck::front() { return m_rounds.front(); }

int RoundCardDeck::size() { return m_rounds.size(); }

// GUI
int RoundCardDeck::width() const { return 300; }

int RoundCardDeck::height() const { return 100; }

QRectF RoundCardDeck::boundingRect() const {
  return QRectF(0, 0, this->width(), this->height());
}

void RoundCardDeck::paint(QPainter *painter,
