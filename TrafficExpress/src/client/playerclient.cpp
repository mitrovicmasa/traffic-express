...
std::vector<QString> PlayerClient::getNames() { return m_names; }

std::vector<bool> PlayerClient::getReadys() { return m_ready; }

bool PlayerClient::getHost() { return m_isHost; }

int PlayerClient::getIndex() { return m_index; }

QString PlayerClient::getUsername() { return m_username; }

int PlayerClient::getPlayerCount() { return m_ready.size(); }

PlayerPerspective *PlayerClient::getPlayerPerspective() { return m_pp; }

void PlayerClient::setUsername(QString name) { m_username = name; }

bool PlayerClient::allReady() {
  return std::accumulate(m_ready.begin(), m_ready.end(), true,
...
void PlayerClient::onReadyRead() {
  ...
  if (message.startsWith("host")) {
    ...
    if (m_index > dcIndex) m_index--;
    emit changeStartColor();
  }
  this->sendMessage(" ");
}

void PlayerClient::onDisconnected() { QApplication::quit(); }
...
void PlayerClient::onClickedStart() {
  ...
  std::vector<BanditType> allPossibleBandits = {
      BanditType::PICKPOCKET,   BanditType::SEDUCTRESS,
      BanditType::STUDENT,      BanditType::RETIREE,
      BanditType::HOMELESS_MAN, BanditType::BUSINESS_WOMAN};
  std::vector<BanditType> randomlySelectedBandits;
  unsigned seed = std::chrono::system_clock::now().time_since_epoch().count();
  std::sample(allPossibleBandits.begin(), allPossibleBandits.end(),
              std::back_inserter(randomlySelectedBandits),
              this->getPlayerCount(), std::default_random_engine(seed));
  ...
  qDebug() << "QVaraint string"
           << doc.toJson(QJsonDocument::JsonFormat::Indented);
...
void PlayerClient::onPlayerChoseWagon(int playerIndex, int wagonIndex) {
  this->sendMessage("pcw:" + QString::number(playerIndex) + ":" +
                    QString::number(wagonIndex));
}

void PlayerClient::onPlayerPlayedCard(int playerIndex, int CardIndex) {
  this->sendMessage("ppc:" + QString::number(playerIndex) + ":" +
                    QString::number(CardIndex));
}
...
void PlayerClient::onActionRobberySignal(int treasureIndex, int wagonIndex) {
  this->sendMessage("ars:" + QString::number(treasureIndex) + ":" +
                    QString::number(wagonIndex));
}

void PlayerClient::onActionPunchSignal(int treasureIndex, int playerIndex,
                                       int wagonIndex) {
  this->sendMessage("aps:" + QString::number(treasureIndex) + ":" +
