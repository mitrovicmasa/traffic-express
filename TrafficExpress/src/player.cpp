...
// Constructors
Player::Player() { setFlags(GraphicsItemFlag::ItemIsSelectable); }
...
Player::Player(bool isItMyMove, BanditType id, Hand *h, Deck *d, Deck *bcd,
               int pos, bool roof, TreasureChest tc)
    : m_isItMyMove(isItMyMove),
      m_id(id),
      m_hand(new Hand(h->getCards())),
      m_deck(new Deck(d->getCards())),
      m_bulletDeck(new Deck(bcd->getCards())),
      m_positionInTrain(pos),
      m_roof(roof),
      m_treasure(TreasureChest(tc)) {
  connect(m_hand, &Hand::clickedCardInHand, this, &Player::onClickedCardInHand);
  connect(m_deck, &Deck::clickedCardInDeck, this, &Player::onClickedCardInDeck);
...
BanditType Player::id() const { return m_id; }

Hand *Player::hand() { return m_hand; }

Deck *Player::deck() { return m_deck; }

Deck *Player::bullet_deck() { return m_bulletDeck; }

int Player::positionInTrain() const { return m_positionInTrain; }

bool Player::roof() const { return m_roof; }

TreasureChest &Player::treasure() { return m_treasure; }

bool Player::isItMyMove() const { return m_isItMyMove; }
...
void Player::setRoof(bool newRoof) { m_roof = newRoof; }

void Player::setMyMove(bool v) { m_isItMyMove = v; }

// Other methods

bool Player::isBulletDeckEmpty() const { return m_bulletDeck->empty(); }

int Player::countAmountOfTreasure() const {
  return std::accumulate(
      m_treasure.cbegin(), m_treasure.cend(), 0,
      [](int acc, auto treasure) { return acc + treasure->getValue(); });
}
...
void Player::shuffleDeck(int seed) { m_deck->shuffleDeck(seed); }
...
void Player::fromVariant(const QVariant &variant) {
  ...
  m_bulletDeck = new Deck();
  for (int i = 1; i <= 6; i++) m_bulletDeck->push_back(new BulletCard(m_id, i));
...
std::string Player::toString() const {
  std::string positionInWagon =
      m_roof ? "Bandit is on the roof" : "Bandit is in the wagon";
  int currentAmountOfTreasure = countAmountOfTreasure();

  std::string cardsInDeck = "";
  for (auto x : (m_deck->getCards())) cardsInDeck += x->toString() + "\n";

  std::string cardsInHand = "";
  for (auto x : m_hand->getCards()) cardsInHand += x->toString() + "\n";

  return "Bandit: " + ::toString(m_id) + "\n" +
         "Position in train: " + std::to_string(m_positionInTrain) + "\n" +
         "Current amount of treasure: " +
         std::to_string(currentAmountOfTreasure) + "\n" +
         "Current number of cards in deck: " + std::to_string(m_deck->size()) +
         "\n" + "Current number of bullets in bullet deck: " +
         std::to_string(m_bulletDeck->size()) + "\n" + positionInWagon +
         "\nCards in deck:\n" + cardsInDeck + "\n" + "\nCards in hand:\n" +
         cardsInHand + "\n";
}
...
QRectF Player::boundingRect() const { return QRectF(0, 0, 50, 50); }
...
int Player::width() const { return 25; }

int Player::height() const { return 35; }
